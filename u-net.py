...
from keras.optimizers import SGD,Nadam,AdamAccum,RMSprop,Adam
from keras.layers.advanced_activations import PReLU, ELU

from BatchRenormalization import BatchRenormalization
from losses import dice_coeff,dice_loss,bce_dice_loss,weighted_bce_loss,weighted_bce_dice_loss
#from NadamAccum import NadamAccum
...
def get_unet_256(input_shape=(256, 256, 3),
    ...
    model.compile(optimizer=SGD(lr=0.005, momentum=0.9,nesterov=True), loss=bce_dice_loss, metrics=[dice_loss])
...
def get_unet_512(input_shape=(512, 512, 3),
    ...
    model.compile(optimizer=Nadam(lr=0.002,beta_1=0.9,beta_2=0.999,epsilon=1e-08,schedule_decay=0.004), loss=bce_dice_loss, metrics=[dice_loss])
...
def get_unet_1280(input_shape=( 1280, 1920, 3),
                  num_classes=1):
    inputs = Input(shape=input_shape)
    # 1024
    #1280
    ...
    #model.compile(optimizer=RMSprop(lr=0.0001), loss=bce_dice_loss, metrics=[dice_coeff])
    model.compile(optimizer=SGD(lr=0.001,momentum=0.99,nesterov=True), loss=bce_dice_loss, metrics=[dice_coeff])
